...
\begin{itemize}
\item \texttt{lms\_equalizer.m}: Implements the complex LMS adaptive FIR equalizer. Performs the core adaptation loop: computes equalizer output $\hat{x}[n]$, determines the desired signal $d[n]$ (training or decision--directed), computes the error $e[n]$, and updates coefficients via $\mathbf{c}[n+1] = \mathbf{c}[n] + \mu \mathbf{y}[n]e^*[n]$. Returns the equalized output, error signal, coefficient history, and MSE learning curve.
\end{itemize}
